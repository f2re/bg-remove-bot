...
- 📸 Автоматическое удаление фона с изображений
  - **Режим Фото**: быстрая обработка с белым фоном
  - **Режим Документ**: высокое качество с прозрачным фоном (PNG)
...
#### 📸 Два режима отправки изображений:

**1. Отправка как Фото (с компрессией)**
- Отправьте изображение обычным способом (как фото)
- Telegram автоматически сжимает изображение
- Результат: изображение на **белом фоне**
- Подходит для: быстрой обработки, веб-использования, когда размер файла важен

**2. Отправка как Документ (без компрессии)**
- Нажмите 📎 (скрепка) → выберите файл → отправьте как документ
- Без сжатия, сохраняется оригинальное качество
- Результат: PNG изображение с **прозрачным фоном**
- Подходит для: профессионального использования, печати, дизайна, когда важно качество

#### Процесс обработки:

1. Пользователь отправляет фото боту (как фото или документ)
...
5. Возвращает обработанное изображение (на белом фоне для фото, с прозрачным фоном для документа)
